...
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY!, {
  apiVersion: '2025-02-24.acacia', // Fixed to match expected type
});
const webhookSecret = process.env.STRIPE_WEBHOOK_SECRET!;

export async function POST(req: Request) {
  const body = await req.text();
  const headersObj = await headers(); // Await headers()
  ...
  const donation = {
    ...
    description: "Stripe",
    type: 'Cash' as import('@/entities/donation').type, // Use correct type
